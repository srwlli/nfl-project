 ...
 * Updates the injuries table with latest player availability.
 *
...
function transformInjuryData(injury, teamId) {
  ...
  return {
    ...
    body_part: injury.details?.detail || injury.details?.side || null,
    date_reported: injury.date || new Date().toISOString().split('T')[0],
    long_comment: injury.longComment || injury.details?.fantasyStatus || null,
...
async function scrapeInjuries(specificTeamId = null) {
  try {
    ...
    if (allInjuries.length > 0) {
      logger.info('Updating injuries table...')
